...
import { admins, breakingNews, contentPages, settings, statistics } from './db/schema.js'
...
async function main(): Promise<void> {
  ...
  const existingPages = await db.query.contentPages.findMany()
  if (existingPages.length === 0) {
    await db.insert(contentPages).values(
      seedPages.map((p) => ({ ...p, isPublished: 1 })),
    )
  ...
  const existingNews = await db.query.breakingNews.findMany()
